    ...
    ig.realtime.on('message', async (data) => { 
        ...
        //console.log(process.env.ENABLE_BOT);
            ...
            if( !String(data.message.path).endsWith('has_seen') && String(data.message.user_id) !== ig.state.cookieUserId ){
                await console.log(`Msg: ${ig.entity.directThread(data.message.text)}`)
            }  
          if(data.message.text == "ping"){
            await ig.entity.directThread(data.message.thread_id).broadcastText("Pong!")
          }

chatbot.chat(`${data.message.text}`).then(reply => 
ig.entity.directThread(data.message.thread_id).broadcastText(`${reply}`)).catch(e => console.log(e));
